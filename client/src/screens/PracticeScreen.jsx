    ...
    <div className="h-full flex flex-col px-3 py-2 max-w-md lg:max-w-4xl mx-auto">
      <header className="flex items-center justify-between mb-2">
        ...
        <div className="text-center flex-1 mx-2">
          <p className="text-sm uppercase tracking-wider text-ink/70">Practice</p>
          {game.practiceTag && (
            <p className="text-[10px] text-ink/60 truncate">{game.practiceTag}</p>
          )}
        </div>
      ...
      <div className="text-center mb-2 min-h-[24px]">
        {upcomingPlayer ? (
          ...
          <p className="text-sm font-semibold text-ink/80">Set ready, tap Bank</p>
        )}
      </div>
    ...
    <div className="fixed inset-0 bg-black/75 z-40 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="overflow-y-auto -mx-1 px-1 mb-3">
          <p className="text-xs uppercase tracking-wider text-ink/60 mb-1">This set</p>
            ...
            {players.map((gp) => {
              ...
              return (
                <div key={gp.playerId} className="text-sm">
                  ...
                  <ol className="space-y-0.5 pl-5 list-decimal text-ink/80 text-xs">
                    {labels.map((l, i) => (
                      <li key={i}>{l}</li>
                    ))}
                  </ol>
          ...
          {stats && (
            <>
              <p className="text-xs uppercase tracking-wider text-ink/60 mb-1">Session totals</p>
          ...
          {sessionThrows.length > 0 && (
            <>
              <p className="text-xs uppercase tracking-wider text-ink/60 mb-1">Session heatmap</p>
              <Heatmap throws={sessionThrows} />
              <HeatmapLegend />
              <p className="text-[10px] text-ink/50 text-center mt-1">
        ...
        <button
          onClick={onEnd}
          className="w-full mt-3 text-xs text-ink/55 underline underline-offset-4"
        >
          End practice
        </button>
      </div>
